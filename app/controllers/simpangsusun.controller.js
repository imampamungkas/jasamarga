...
exports.create = async (req, res) => {
  ...
  const { i18n, ...simpangsusun } = req.body;
  simpangsusun["postUuid"] = postUuid;
  if (req.body.hasOwnProperty("nama_file")) {
    if (req.body.nama_file) {
      ...
      const timestamp = `simpangsusun-${postUuid}/${new Date().getTime()}`;
      ...
      simpangsusun["nama_file"] = `${timestamp}/${file_name}`;
    }
  }

  // Save Photo in the database
  Simpangsusun.create(simpangsusun)
    .then(async (data) => {
      if (i18n instanceof Array && i18n.length > 0) {
        for (var i = 0; i < i18n.length; i++) {
          i18n[i]["simpangsusunUuid"] = data.uuid;
          await SimpangsusunI18n.create(i18n[i]);
        }
...
exports.update = async (req, res) => {
  ...
  if (req.body.hasOwnProperty("nama_file")) {
    if (req.body.nama_file) {
      ...
      delete simpangsusun.nama_file;
    }
  }
  Simpangsusun.findByPk(uuid)
    .then(async (data) => {
      if (data == null) {
        res.status(404).send({
        ...
        if (simpangsusun.nama_file != null && data.nama_file != null) {
        ...
        if (i18n instanceof Array && i18n.length > 0) {
          for (var i = 0; i < i18n.length; i++) {
            ...
            const [obj, created] = await SimpangsusunI18n.findOrCreate({
              where: { simpangsusunUuid: uuid, lang: lang },
              defaults: trans
            });
            if (!created) {
              obj.update(trans);
        ...
        data.update(simpangsusun).then(async (result) => {
...
exports.updateBulk = async (req, res) => {
  ...
  if (data instanceof Array && data.length > 0) {
    for (var i = 0; i < data.length; i++) {
      ...
      var result = await Simpangsusun.update(data[i], {
        where: { uuid: uuid },
      });
      if (result[0] > 0) {
...
exports.deleteAll = (req, res) => {
  const postUuid = req.params.postUuid;
  Simpangsusun.destroy({
    where: { postUuid: postUuid },
    truncate: false,
  })
    .then((nums) => {
      var path = `public/uploads/simpangsusun-${postUuid}`;
      fs.rm(path, { recursive: true }, (err) => {
        if (err) {
          console.log("err : ", err);
        }
