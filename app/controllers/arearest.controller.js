...
exports.create = async (req, res) => {
  ...
  const { i18n, ...arearest } = req.body;
  arearest["postUuid"] = postUuid;
  if (req.body.hasOwnProperty("nama_file")) {
    if (req.body.nama_file) {
      ...
      const timestamp = `arearest-${postUuid}/${new Date().getTime()}`;
      ...
      arearest["nama_file"] = `${timestamp}/${file_name}`;
    }
  }

  // Save Photo in the database
  Arearest.create(arearest)
    .then(async (data) => {
      if (i18n instanceof Array && i18n.length > 0) {
        for (var i = 0; i < i18n.length; i++) {
          i18n[i]["arearestUuid"] = data.uuid;
          await ArearestI18n.create(i18n[i]);
        }
...
exports.update = async (req, res) => {
  ...
  if (req.body.hasOwnProperty("nama_file")) {
    if (req.body.nama_file) {
      ...
      delete arearest.nama_file;
    }
  }
  Arearest.findByPk(uuid)
    .then(async (data) => {
      if (data == null) {
        res.status(404).send({
        ...
        if (arearest.nama_file != null && data.nama_file != null) {
        ...
        if (i18n instanceof Array && i18n.length > 0) {
          for (var i = 0; i < i18n.length; i++) {
            ...
            const [obj, created] = await ArearestI18n.findOrCreate({
              where: { arearestUuid: uuid, lang: lang },
              defaults: trans
            });
            if (!created) {
              obj.update(trans);
        ...
        data.update(arearest).then(async (result) => {
...
exports.updateBulk = async (req, res) => {
  ...
  if (data instanceof Array && data.length > 0) {
    for (var i = 0; i < data.length; i++) {
      ...
      var result = await Arearest.update(data[i], {
        where: { uuid: uuid },
      });
      if (result[0] > 0) {
...
exports.deleteAll = (req, res) => {
  ...
  Arearest.destroy({
    ...
    .then((nums) => {
      var path = `public/uploads/arearest-${postUuid}`;
      fs.rm(path, { recursive: true }, (err) => {
        if (err) {
          console.log("err : ", err);
        }
