  ...
  Dokumen.findByPk(uuid)
    .then(async (data) => {
      if (data == null) {
        ...
        if (dokumen.cover_file != null && data.cover_file != null) {
        ...
        if (dokumen.dokumen_file != null && data.dokumen_file != null) {
        ...
        if (i18n instanceof Array && i18n.length > 0) {
          for (var i = 0; i < i18n.length; i++) {
            ...
            const [obj, created] = await DokumenI18n.findOrCreate({
              where: { dokumenUuid: uuid, lang: lang },
              defaults: trans
            });
            if (!created) {
              obj.update(trans);
