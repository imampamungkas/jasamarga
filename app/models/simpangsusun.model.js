module.exports = (sequelize, Sequelize) => {
  const Simpangsusun = sequelize.define(
    "simpangsusun",
    {
      uuid: {
        type: Sequelize.UUID,
    ...
    {
      tableName: "simpangsusun",
    }
  );

  Simpangsusun.beforeSave(async (instance, options) => {
    if (instance.changed("thumbnail")) {
      if (instance.thumbnail) {
        await Simpangsusun.update({ thumbnail: false }, { where: { postUuid: instance.postUuid } })
      }
    }
  });
  return Simpangsusun;
